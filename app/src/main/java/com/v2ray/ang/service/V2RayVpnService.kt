...
class V2RayVpnService : VpnService(), ServiceControl {
    ...
    override fun onCreate() {
        ...
        // Request default network early to bind process to physical network
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            try {
                connectivity.requestNetwork(defaultNetworkRequest, defaultNetworkCallback)
            } catch (e: Exception) {
                Log.e(AppConfig.TAG, "Failed to request default network in onCreate", e)
            }
        }

        wakeLock = (getSystemService(Context.POWER_SERVICE) as PowerManager).run {
            newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "${javaClass.name}:wakeLock").apply {
                acquire()
            }
        }
    }
...
//    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            try {
                ...
                Log.e("V2RayVpnService", "Failed to start foreground: ${e.message}")
            }
        }
    ...
    private fun runTun2socks() {
        tun2SocksService = if (MmkvManager.decodeSettingsBool(AppConfig.PREF_USE_HEV_TUNNEL)) {
            TProxyService(
