...
import app.tunnel.vpncommons.utils.DataHolder.bundle
...
//            }
     ...
     */
    private fun subscriptionIdChanged(id: String) {
            ...
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_MEASURE_DELAY_SUCCESS -> {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed({
                        if (intent.getStringExtra("content")?.contains("Success") == true) {
                            val bundle1 = Bundle()
                           ...
                           // getApplication<AngApplication>().toastSuccess(R.string.toast_services_success,+intent.getStringExtra("content"))
                        } else {
                            // Don't stop the VPN if the speed test fails, just log it
                            updateTestResultAction.value?.let {
                                getApplication<AngApplication>().toastError(
                                    it
                                )
                            }
                        }
