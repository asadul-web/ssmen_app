...
object AngConfigManager {
    ...
    @JvmStatic
    fun importCustomizeConfig(server: String?): Boolean {
        return try {
            val config = ServerConfig.create(EConfigType.CUSTOM)
            config.remarks = System.currentTimeMillis().toString()
            config.subscriptionId = ""
            config.fullConfig = Gson().fromJson(server, V2rayConfig::class.java)
            val key = com.v2ray.ang.util.MmkvManager.encodeServerConfig("", config)
    ...
    @JvmStatic
    fun importBatchConfig(server: String): Boolean {

        var count = parseBatchConfig(Utils.decode(server),"",true)
        if (count <= 0) {
            count =  parseBatchConfig(server, "", true)
        }
        if (count > 0) {
    ...
    @JvmStatic
    fun importBatchConfig1(server: String?, subid: String, append: Boolean): Pair<Int, Int> {
