...
/**
 * Converts a Long value (bytes) to a speed string in bit format.
 */
fun Long.toBitSpeedString(): String {
    val bits = this * 8.0
    val units = arrayOf("bit", "kbit", "Mbit", "Gbit")
    var size = bits
    var unitIndex = 0
    // If it's small, force it to at least kbit for better visual consistency
    if (size > 0 && size < 100.0) {
        return String.format("%.2f bit", size)
    }
