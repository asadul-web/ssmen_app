...
object Utils {
    ...
    fun decode(text: String?): String {
        ...
        if (trimmed != text) {
            val decodedTrimmed = tryDecodeBase64(trimmed)
            if (decodedTrimmed != null) return decodedTrimmed
        }
    ...
    fun tryDecodeBase64(text: String?): String? {
        ...
        try {
            ...
            Log.d(AppConfig.TAG, "Not valid standard base64")
        } catch (e: Exception) {
            Log.e(AppConfig.TAG, "Failed to decode standard base64", e)
        }
        
        try {
            val decoded = Base64.decode(text, Base64.NO_WRAP.or(Base64.URL_SAFE))
            if (decoded != null && decoded.isNotEmpty()) {
                return String(decoded, Charsets.UTF_8)
            }
        } catch (e: IllegalArgumentException) {
            Log.d(AppConfig.TAG, "Not valid URL-safe base64")
        } catch (e: Exception) {
