...
import mtkdex.core.build.ssmen.core.PasswordCache;
import mtkdex.core.build.ssmen.logger.hLogStatus;
import mtkdex.core.build.ssmen.thread.rm_001;
import mtkdex.core.build.ssmen.utils.util;

/*Fix by Tknetwork 11-13-24*/
public class dex001 extends Thread implements ConnectionMonitor, InteractiveCallback, ServerHostKeyVerifier, DebugLogger, SettingsConstants
{
    private static final String TAG = dex001.class.getSimpleName();
    ...
    public dex001(dex002 context, OnSSHTunnelListener mListener) {
        mContext = context;
    ...
    protected void startClienteDNSTT() throws Exception {
        ...
        String[] mHost = mConfig.getSecureString(SERVER_KEY).split(":");
        int serverPort = Integer.parseInt(mConfig.getSecureString(SERVER_PORT_KEY));
...
//        int mPort = Integer.parseInt(mConfig.getSecureString(SERVER_PORT_KEY));
        ...
        try {
            conectar2(mHost[0], serverPort,prxAdrss);
    ...
    protected void conectar2(String servidor, int porta , String[] prxAdrss) throws Exception {
        if (!mStarting) {
            throw new Exception();
        }
        try {
            ...
            addProxy2(mConnection,prxAdrss[0],Integer.parseInt(prxAdrss[1]));
    ...
    private void addProxy2(Connection conn, String mProxy, int mPort) {
...
//        addLogInfo(mConfig.getSecureString(CUSTOM_PAYLOAD_KEY));
//        addLogInfo(String.valueOf(pType));
