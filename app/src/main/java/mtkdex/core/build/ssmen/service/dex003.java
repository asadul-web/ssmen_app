...
import mtkdex.core.build.ssmen.core.ConfigParser;
import mtkdex.core.build.ssmen.core.Connection;
import mtkdex.core.build.ssmen.core.VpnProfile;
import mtkdex.core.build.ssmen.core.vpnutils.VpnUtils;
import mtkdex.core.build.ssmen.logger.hLogStatus;
import mtkdex.core.build.ssmen.thread.HTTPInjectorThread;
import mtkdex.core.build.ssmen.thread.rm_004;
...
@Obfuscate
public class dex003 extends VpnService implements VpnUtils.VPNProtectListener, SettingsConstants, Callback, rm_004.EventReceiver {
    
    // onStartCommand is defined later in the class
    ...
    public static final String ACTION_FORCE_STOP = "net.openvpn.openvpn.FORCE_STOP";
    ...
    private static final String TAG = "dex003";
    public static final int log_deque_max = 250;
    private boolean active = false;
    private final ArrayDeque<EventReceiver> clients = new ArrayDeque<EventReceiver>();
    ...
    private final ArrayDeque<LogMsg> log_deque = new ArrayDeque<LogMsg>();
    private final IBinder mBinder = new LocalBinder();
    private Handler mHandler;
    private rm_004 mThread;
    ...
    public static class Challenge {
        ...
        public String toString() {
            Object[] objArr = new Object[dex003.EV_PRIO_HIGH];
            objArr[dex003.GCI_REQ_ESTABLISH] = this.challenge;
            objArr[dex003.MSG_EVENT] = this.echo;
            objArr[dex003.MSG_LOG] = this.response_required;
    ...
    private static class DynamicChallenge {
        ...
        public String toString() {
            Object[] objArr = new Object[dex003.EV_PRIO_HIGH];
            objArr[dex003.GCI_REQ_ESTABLISH] = this.challenge.toString();
            objArr[dex003.MSG_EVENT] = this.cookie;
            objArr[dex003.MSG_LOG] = this.expires;
    ...
    public static class EventMsg {
        ...
        public int flags = dex003.GCI_REQ_ESTABLISH;
        public int icon_res_id = -1;
        public String info;
        public String name;
        public int priority = F_ERROR;
        public String profile_override;
        public int progress = dex003.GCI_REQ_ESTABLISH;
        public int res_id = -1;
        ...
        public void mReconnect() {
            if (!mReconnecting && HTTPInjectorThread.f!=200 && dex002.isVPNRunning()) {
                new Handler(Looper.getMainLooper()).postDelayed(() -> {
                    hLogStatus.clearLog();
                    MainApplication.getApp().startService(new Intent(MainApplication.getApp(), dex002.class).setAction(dex002.RESTART_SERVICE));
                }, 1000);
            }
        }
        ...
        public String toString() {
            ...
            objArr[dex003.GCI_REQ_ESTABLISH] = this.name;
            buffer.append(String.format("EVENT: %s", objArr));
            String stateMsg = MainApplication.getApp().getString(hLogStatus.getLocalizedState(hLogStatus.getLastState()));
            ...
            if (!dex002.isVPNRunning() || stateMsg.equals(MainApplication.getApp().getResources().getString(R.string.state_reconnecting))){
                mReconnecting = true;
                reconnectHandler.removeCallbacks(reconnectRunnable);
            }
            if (!this.info.isEmpty()) {
                objArr = new Object[F_ERROR];
                objArr[dex003.GCI_REQ_ESTABLISH] = this.info;
                buffer.append(String.format(" info='%s'", objArr));
            }
            if (this.transition != Transition.NO_CHANGE) {
                objArr = new Object[F_ERROR];
                objArr[dex003.GCI_REQ_ESTABLISH] = this.transition;
    ...
    public class Profile {
        ...
        private Profile(String location_arg, String filename, boolean filename_is_url_encoded_profile_name, ClientAPI_EvalConfig ec) {
            ...
            if (filename_is_url_encoded_profile_name) {
                ...
                try {
                    ...
                    Log.e(dex003.TAG, "UnsupportedEncodingException when decoding profile filename", e);
                }
            } else {
                this.name = filename;
            }
            ...
            if (!filename_is_url_encoded_profile_name) {
                ...
                if (this.autologin && !is_friendly && f2 == null) {
                    n.append(dex003.this.getText(R.string.autologin_suffix).toString());
                }
                if (!(loc == null && f2 == null)) {
                    n.append(" [");
                    if (loc != null) {
            ...
            int n2 = (int) sev.size();
            for (int i = dex003.GCI_REQ_ESTABLISH; i < n2; i += dex003.MSG_EVENT) {
            ...
            this.external_pki_alias = dex003.this.prefs.get_string_by_profile(this.name, "epki_alias");
        }
        ...
        public String get_type_string() {
            if (get_autologin()) {
                return dex003.this.getText(R.string.profile_type_autologin).toString();
            }
            if (get_epki()) {
                return dex003.this.getText(R.string.profile_type_epki).toString();
            }
            return dex003.this.getText(R.string.profile_type_standard).toString();
        }
        ...
        public boolean server_list_defined() {
            return !this.server_list.list.isEmpty();
        }

        public boolean userlocked_username_defined() {
            return !this.userlocked_username.isEmpty();
        }
        ...
        private void persist_epki_alias(String epki_alias) {
            ...
            dex003.this.prefs.set_string_by_profile(this.name, "epki_alias", epki_alias);
            dex003.this.jellyBeanHackPurge();
        }
        ...
        private void invalidate_epki_alias(String epki_alias) {
            if (this.external_pki_alias != null && this.external_pki_alias.equals(epki_alias)) {
                ...
                dex003.this.prefs.delete_key_by_profile(this.name, "epki_alias");
                dex003.this.jellyBeanHackPurge();
            }
        }
        ...
        public String toString() {
            ...
            objArr[dex003.GCI_REQ_ESTABLISH] = this.name;
            objArr[dex003.MSG_EVENT] = this.orig_filename;
            objArr[dex003.MSG_LOG] = this.userlocked_username;
            objArr[dex003.EV_PRIO_HIGH] = this.autologin;
            objArr[4] = this.external_pki;
    ...
    private static class ProfileFN {
        ...
        public static String strip_ovpn_ext(String fn) {
            ...
            return fn.substring(dex003.GCI_REQ_ESTABLISH, fn.length() - 5);
        }
        ...
        public static String encode_profile_fn(String name) {
            try {
                ...
                Log.e(dex003.TAG, "UnsupportedEncodingException when encoding profile filename", e);
                return null;
            }
        }
    }
    ...
    public class ProfileList extends ArrayList<Profile> {
        ...
        private void load_profiles(String location) throws IOException {
            ...
            for (String fn : fnlist) {
                if (ProfileFN.has_ovpn_ext(fn)) {
                    try {
                        String profile_content = dex003.this.read_file(location, fn);
                        ...
                        if (ec.getError()) {
                            str = dex003.TAG;
                            objArr = new Object[dex003.MSG_LOG];
                            objArr[dex003.GCI_REQ_ESTABLISH] = fn;
                            objArr[dex003.MSG_EVENT] = ec.getMessage();
                            Log.i(str, String.format("PROFILE: error evaluating %s: %s", objArr));
                        } else {
                            add(new Profile(location, fn, true, ec));
                        }
                    } catch (IOException e) {
                        try {
                            str = dex003.TAG;
                            objArr = new Object[dex003.MSG_LOG];
                            objArr[dex003.GCI_REQ_ESTABLISH] = fn;
                            objArr[dex003.MSG_EVENT] = storage_title;
                            Log.i(str, String.format("PROFILE: error reading %s from %s", objArr));
                        } catch (Exception e2) {
                            Log.e(dex003.TAG, "PROFILE: error enumerating assets", e2);
                            return;
                        }
                    }
                }
        ...
        public String[] profile_names() {
            ...
            for (int i = dex003.GCI_REQ_ESTABLISH; i < size(); i += dex003.MSG_EVENT) {
        ...
        private void invalidate_epki_alias(String epki_alias) {
            for (Profile profile : this) {
                profile.invalidate_epki_alias(epki_alias);
            }
        }
    ...
    public static class ServerEntry {
        ...
        public String toString() {
            Object[] objArr = new Object[dex003.MSG_LOG];
            objArr[dex003.GCI_REQ_ESTABLISH] = this.server;
            objArr[dex003.MSG_EVENT] = this.friendly_name;
    ...
    public static class ServerList {
        ...
        public String toString() {
            StringBuilder buffer = new StringBuilder();
            for (ServerEntry serverEntry : this.list) {
                buffer.append(serverEntry.toString()).append(",");
            }
            return buffer.toString();
        }
    }

    private class TunBuilder extends Builder implements rm_004.TunBuilder {
        private TunBuilder() {
            super();
        }
        ...
        public boolean tun_builder_set_remote_address(String address, boolean ipv6) {
            try {
                Object[] objArr = new Object[dex003.MSG_LOG];
                objArr[dex003.GCI_REQ_ESTABLISH] = address;
                objArr[dex003.MSG_EVENT] = ipv6;
                Log.d(dex003.TAG, String.format("BUILDER: set_remote_address %s ipv6=%b", objArr));
                return true;
            } catch (Exception e) {
        ...
        public boolean tun_builder_add_address(String address, int prefix_length, String gateway, boolean ipv6, boolean net30) {
            try {
                Log.d(dex003.TAG, String.format("BUILDER: add_address %s/%d %s ipv6=%b net30=%b", new Object[]{address, Integer.valueOf(prefix_length), gateway, Boolean.valueOf(ipv6), Boolean.valueOf(net30)}));
        ...
        public boolean tun_builder_reroute_gw(boolean ipv4, boolean ipv6, long flags) {
            try {
                Object[] objArr = new Object[dex003.EV_PRIO_HIGH];
                objArr[dex003.GCI_REQ_ESTABLISH] = ipv4;
                objArr[dex003.MSG_EVENT] = ipv6;
                objArr[dex003.MSG_LOG] = flags;
                Log.d(dex003.TAG, String.format("BUILDER: reroute_gw ipv4=%b ipv6=%b flags=%d", objArr));
                if ((65536 & flags) != 0) {
                    return true;
                }
                if (ipv4) {
                    addRoute("0.0.0.0", dex003.GCI_REQ_ESTABLISH);
                }
                if (!ipv6) {
                    return true;
                }
                addRoute("::", dex003.GCI_REQ_ESTABLISH);
                return true;
            } catch (Exception e) {
        ...
        public boolean tun_builder_add_route(String address, int prefix_length, boolean ipv6) {
            try {
                Object[] objArr = new Object[dex003.EV_PRIO_HIGH];
                objArr[dex003.GCI_REQ_ESTABLISH] = address;
                objArr[dex003.MSG_EVENT] = prefix_length;
                objArr[dex003.MSG_LOG] = ipv6;
                Log.d(dex003.TAG, String.format("BUILDER: add_route %s/%d ipv6=%b", objArr));
        ...
        public boolean tun_builder_exclude_route(String address, int prefix_length, boolean ipv6) {
            try {
                ...
                objArr[dex003.MSG_EVENT] = prefix_length;
                objArr[dex003.MSG_LOG] = ipv6;
                Log.d(dex003.TAG, String.format("BUILDER: exclude_route %s/%d ipv6=%b (NOT IMPLEMENTED)", objArr));
                return true;
            } catch (Exception e) {
        ...
        public boolean tun_builder_add_dns_server(String address, boolean ipv6) {
            try {
                String str = dex003.TAG;
                Object[] objArr = new Object[dex003.MSG_LOG];
                objArr[dex003.GCI_REQ_ESTABLISH] = address;
                objArr[dex003.MSG_EVENT] = Boolean.valueOf(ipv6);
        ...
        public boolean tun_builder_add_search_domain(String domain) {
            try {
                Object[] objArr = new Object[dex003.MSG_EVENT];
                objArr[dex003.GCI_REQ_ESTABLISH] = domain;
                Log.d(dex003.TAG, String.format("BUILDER: add_search_domain %s", objArr));
        ...
        public boolean tun_builder_set_mtu(int mtu) {
            try {
                Object[] objArr = new Object[dex003.MSG_EVENT];
                objArr[dex003.GCI_REQ_ESTABLISH] = mtu;
                Log.d(dex003.TAG, String.format("BUILDER: set_mtu %d", objArr));
                setMtu(mtu);
                return true;
            } catch (Exception e) {
        ...
        public boolean tun_builder_set_session_name(String name) {
            try {
                Object[] objArr = new Object[dex003.MSG_EVENT];
                objArr[dex003.GCI_REQ_ESTABLISH] = name;
                Log.d(dex003.TAG, String.format("BUILDER: set_session_name %s", objArr));
        ...
        public int tun_builder_establish() {
            try {
                Log.d(dex003.TAG, "BUILDER: establish");
                PendingIntent pi = ConfigUtil.getPendingIntent(dex003.this);
                if (pi != null) {
        ...
        public void tun_builder_teardown(boolean disconnect) {
            try {
                Object[] objArr = new Object[dex003.MSG_EVENT];
                objArr[dex003.GCI_REQ_ESTABLISH] = disconnect;
                Log.d(dex003.TAG, String.format("BUILDER: teardown disconnect=%b", objArr));
            } catch (Exception e) {
        ...
        private void log_error(String meth_name, Exception e) {
            Object[] objArr = new Object[dex003.MSG_LOG];
            objArr[dex003.GCI_REQ_ESTABLISH] = meth_name;
            objArr[dex003.MSG_EVENT] = e.toString();
            Log.d(dex003.TAG, String.format("BUILDER_ERROR: %s %s", objArr));
        }
    }
    ...
    public void onCreate() {
        ...
        // Start foreground moved to connect_action to avoid showing on app open
        // startForegroundService();
        
        crypto_self_test();
        this.mHandler = new Handler(Looper.getMainLooper(), this);
        populate_event_info_map();
        this.prefs = new PrefUtil(MainApplication.getDefaultSharedPreferences());
        this.pwds = new PasswordUtil(MainApplication.getDefaultSharedPreferences());
    ...
    private void startForegroundService() {
        // Only start foreground if we are actually connecting an OVPN profile
        // Added extra safety check to ensure it doesn't show for V2Ray
        if (mConfig == null || !SERVER_TYPE_OVPN.equals(mConfig.getServerType()) || dex002.isVPNRunning()) {
            return;
        }
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                channel.setDescription("VPN Service Notification");
            ...
            Notification notification = new Notification.Builder(this, 
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.O ? "vpn_service_channel" : null)
            ...
            Log.d(TAG, "SERV: Started foreground service successfully");
        } catch (Exception e) {
            Log.e(TAG, "SERV: Failed to start foreground service", e);
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId)  {
        ...
        switch (action) {
            ...
            case ACTION_FORCE_STOP:
                try {
                    Log.d(TAG, "ExitDebug: dex003 received FORCE_STOP action, releasing VPN resources");
                    
                    // Call onDestroy to clean up all VPN resources
                    try {
                        // Call onDestroy to clean up resources
                        Log.d(TAG, "ExitDebug: dex003 calling onDestroy() to clean up resources");
                        onDestroy();
                        Log.d(TAG, "ExitDebug: dex003 onDestroy() completed successfully");
                    } catch (Exception e) {
                        Log.e(TAG, "ExitDebug: Error in onDestroy during force stop", e);
                    }
                    
                    // Stop self and ensure we don't restart
                    Log.d(TAG, "ExitDebug: dex003 calling stopSelf()");
                    stopSelf();
                    Log.d(TAG, "ExitDebug: dex003 stopSelf() completed, returning START_NOT_STICKY");
                } catch (Exception e) {
                    Log.e(TAG, "ExitDebug: Error during force stop", e);
                }
                return START_NOT_STICKY;
            ...
            case ACTION_PAUSE:
                if (hLogStatus.isTunnelActive()) {
                    dex003.this.network_pause();
                }
                break;
            case ACTION_RESUME:
                if (hLogStatus.isTunnelActive()) {
                    dex003.this.network_resume();
                }
                break;
            case ACTION_RECONNECT:
                if (intent.getStringExtra(prefix + ".SECONDS")!=null){
                    ...
                    if (hLogStatus.isTunnelActive()) {
                        dex003.this.network_reconnect(second);
                    }
                    break;
                }
                if (hLogStatus.isTunnelActive()) {
                    dex003.this.network_reconnect(0);
                }
                break;
        }
        return START_STICKY;
    ...
    private String cert(JSONObject server,String server_ip,String server_port) throws JSONException {
        ...
        if (isCustomConfig){
            String multi = c_01.decrypt(server.getString("ovpnCertificate"));
            if (multi.contains("remote ")){
                return multi;
            }
            return multi + "\nremote " + server_ip + " " + server_port + "\n";
        }
        String single = c_01.decrypt(mPref.getString(OPEN_VPN_CERT, ""));
    ...
    private boolean reload_profile_action(String profile_content) {
        try {
            ...
            for (int i = 0; i < s_js.length(); i++) {
                ...
                String server_ip = c_01.decrypt(server.getString("ServerIP"));
                ...
                String config = vp.getConfigFile(dex003.this, true);
    ...
    private void import_profile_via_path_action(String prefix, Intent intent) {
        ...
        if (status.equals("PROFILE_MERGE_SUCCESS")) {
            import_profile(mc.getProfileContent(), mc.getBasename(), false);
            return;
        }
        gen_event(MSG_EVENT, status, mc.getErrorText());
    }

    private void import_profile_action(String prefix, Intent intent) {
        import_profile(intent.getStringExtra(prefix + ".CONTENT"), intent.getStringExtra(prefix + ".FILENAME"), intent.getBooleanExtra(prefix + ".MERGE", false));
    }

    private boolean import_profile(String profile_content, String filename, boolean merge) {
        if (ProfileFN.has_ovpn_ext(filename) && c_01.dirname(filename) == null) {
            if (merge) {
            ...
            try {
                c_01.writeFileAppPrivate(this, prof.get_filename(), profile_content);
                String profile_name = prof.get_name();
                this.pwds.remove("auth", profile_name);
                this.pwds.remove("pk", profile_name);
                refresh_profile_list();
                gen_event("PROFILE_IMPORT_SUCCESS", profile_name, profile_name);
                return true;
            } catch (IOException e) {
    ...
    private void delete_profile_action(String prefix, Intent intent) throws IOException {
    ...
    private void rename_profile_action(String prefix, Intent intent) throws IOException {
        String profile_name = intent.getStringExtra(prefix + ".PROFILE");
        ...
        if (c_01.renameFile(from_path, to_path)) {
            refresh_profile_list();
            ...
            this.pwds.remove("auth", profile_name);
            this.pwds.remove("pk", profile_name);
            gen_event("PROFILE_RENAME_SUCCESS", new_profile.get_name(), new_profile.get_name());
            return;
        }
        Object[] objArr2 = new Object[MSG_LOG];
        objArr2[GCI_REQ_ESTABLISH] = from_path;
        objArr2[MSG_EVENT] = to_path;
        Log.d(TAG, String.format("PROFILE_RENAME_FAILED: rename operation from='%s' to='%s'", objArr2));
        gen_event(MSG_EVENT, "PROFILE_RENAME_FAILED", profile_name);
    }
    ...
    private void connect_action(final String prefix, final Intent intent) {
        if (this.active) {
            stop_thread();
            new Handler(Looper.getMainLooper(), dex003.this).postDelayed(() -> {
                try {
                    dex003.this.do_connect_action(prefix, intent, false);
                } catch (IOException e) {addLogInfo(e.toString());}
            }, 2000);
        } else {
            try {
                do_connect_action(prefix, intent, false);
            } catch (IOException e) {
    ...
    private void do_connect_action(String prefix, Intent intent, boolean proxy_retry) throws IOException {
        startForegroundService();
        String profile_name = intent.getStringExtra(prefix + ".PROFILE");
        ...
        try {
            ...
            Object[] objArr = new Object[MSG_EVENT];
            objArr[GCI_REQ_ESTABLISH] = profile_content.length();
            ...
            start_connection(profile, profile_content, gui_version, server, proto, ipv6, conn_timeout, username, password, cache_password, pk_password, response, epki_alias, compression_mode);
        } catch (IOException e) {
            Object[] objArr2 = new Object[MSG_LOG];
    ...
    private String getIpOrHost(String config) {
        ...
        for (String line : split) {
            if (line.toLowerCase().trim().startsWith("remote ")) {
    ...
    private String getPort(String config) {
        ...
        for (String line : split) {
            if (line.toLowerCase().trim().startsWith("remote ")) {
                ...
                // Port is optional, default is 1194
                if (split_line.length > 2) {
                    String potentialPort = split_line[2];
                    // Check if it's actually a port number (not protocol like tcp-client)
                    try {
                        Integer.parseInt(potentialPort);
                        return potentialPort;
                    } catch (NumberFormatException e) {
                        // Not a valid port number, might be protocol
                        Log.d(TAG, "Port not specified or invalid in remote line, using default");
                        return "1194";
                    }
                } else {
    ...
    private void start_connection(Profile profile, String profile_content, String gui_version, String server, String proto, String ipv6, String conn_timeout, String username, String password, boolean cache_password, String pk_password, String response, String epki_alias, String compression_mode) {
        if (this.active) {
            return;
        }
        rm_004 thread = new rm_004();
        ClientAPI_Config config = new ClientAPI_Config();
        ...
        Log.i(TAG, String.format(str2, objArr));
    ...
    public IBinder onBind(Intent intent) {
        if (intent == null || !Objects.equals(intent.getAction(), ACTION_BIND)) {
            Object[] objArr = new Object[MSG_EVENT];
            objArr[GCI_REQ_ESTABLISH] = intent;
            Log.d(TAG, String.format("SERV: onBind SUPER intent=%s", objArr));
    ...
    public void client_attach(EventReceiver evr) {
        ...
        Object[] objArr = new Object[MSG_EVENT];
        objArr[GCI_REQ_ESTABLISH] = this.clients.size();
        Log.d(TAG, String.format("SERV: client attach n_clients=%d", objArr));
    }
    ...
    public void client_detach(EventReceiver evr) {
        ...
        Object[] objArr = new Object[MSG_EVENT];
        objArr[GCI_REQ_ESTABLISH] = this.clients.size();
        Log.d(TAG, String.format("SERV: client detach n_clients=%d", objArr));
    }
    ...
    public void refresh_profile_list() throws IOException {
        ...
        for (Profile p : pl) {
            ...
            Log.d(TAG, String.format("SERV: %s", objArr));
        }
    ...
    public boolean onUnbind(Intent intent) {
        ...
        Log.d(TAG, String.format("SERV: onUnbind called intent=%s", objArr));
    ...
    private void gen_event(String name, String extra_info, String profile_override) {
        gen_event(dex003.GCI_REQ_ESTABLISH, name, extra_info, profile_override, null);
    }
        ...
        switch (msg.what) {
            case MSG_EVENT :
                ...
                if (evm.res_id == R.string.state_auth_failed) {
                    ...
                    startService(new Intent(dex003.this, dex002.class).setAction(dex002.STOP_SERVICE).putExtra("stateSTOP_SERVICE",getResources().getString(R.string.state_auth_failed)));
                    if (this.current_profile != null) {
            ...
            case MSG_LOG /*2*/:
                ...
                Log.i(TAG, String.format("LOG: %s", objArr2));
                log_message(lm);
                break;
            default:
    ...
    private void addLogInfo(String msg){
        ...
        if (msg.equals("0")|| msg.isEmpty()){
            if (util.isNetworkAvailable(dex003.this)){
                hLogStatus.clearLog();
                startService(new Intent(dex003.this, dex002.class).setAction(dex002.RESTART_SERVICE));
            }
        }
    ...
    public boolean socket_protect(int socket) {
        ...
        Object[] objArr = new Object[MSG_LOG];
        objArr[GCI_REQ_ESTABLISH] = socket;
        objArr[MSG_EVENT] = status;
        Log.d(TAG, String.format("SOCKET PROTECT: fd=%d protected status=%b", objArr));
        return status;
    }
    ...
    public boolean pause_on_connection_timeout() {
        ...
        Object[] objArr = new Object[MSG_EVENT];
        objArr[GCI_REQ_ESTABLISH] = ret;
        Log.d(TAG, String.format("pause_on_connection_timeout %b", objArr));
        return ret;
    }

    public rm_004.TunBuilder tun_builder_new() {
    ...
    public void event(ClientAPI_Event event) {
        ...
        if (evi != null) {
            ...
            evm.res_id = R.string.state_unknown;
            if (util.isNetworkAvailable(dex003.this) && dex002.isVPNRunning()){
                startService(new Intent(dex003.this, dex002.class).setAction(dex002.RECONNECT_SERVICE));
            }
        }
        this.mHandler.sendMessage(this.mHandler.obtainMessage(MSG_EVENT, evm));
    }
    ...
    public void done(ClientAPI_Status status) {
        ...
        Log.d(TAG, String.format("EXIT: connect() exited, err=%b, msg='%s'", objArr));
        log_stats();
        if (err) {
            ...
            if (sv != null) {
                for (int i = GCI_REQ_ESTABLISH; i < sn.length; i += MSG_EVENT) {
                    ...
                    if (value > 0) {
                        ...
                        objArr[MSG_EVENT] = value;
                        Log.i(TAG, String.format("STAT %s=%s", objArr));
                    }
                }
            }
        }
    ...
    public String read_file(String location, String filename) throws IOException {
        if (location.equals("bundled")) {
            return c_01.readAsset(this, filename);
        }
        if (location.equals("imported")) {
            return c_01.readFileAppPrivate(this, filename);
        }
        throw new InternalError();
    }
