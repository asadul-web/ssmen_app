...
@Obfuscate
public class rm_003 extends Thread implements SettingsConstants {
    ...
    public rm_003(dex002 service, OnUDPListener mListener) {
    ...
    public void run() {
        try {
            ...
            content = jo.toString();
            config.setServerHost(c_01.encrypt(server));
            ...
            StreamGobbler.OnLineListener onLineListener = log -> {
                if (log.contains("Connected")) {
                    ...
                    addLog(log.split("INFO]")[1].replace(config.getSecureString(SERVER_KEY),c_01.hideSTR(config.getSecureString(SERVER_KEY))).replace("0mConnected","<b>Connected</b>").replace("[","").replace("]"," "));
                    ...
                    service.startService(new Intent(service, dex002.class).setAction(dex002.STOP_SERVICE).putExtra("stateSTOP_SERVICE",service.getResources().getString(R.string.state_auth_failed)));
